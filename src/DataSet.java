...
public class DataSet{
	...
	public DataSet(DataSet d){
		...
		this.ground_truth = d.ground_truth;
		this.vList = d.vList;
		this.V = d.V;
	}
		...
		List<Vector> randomized_centers = new ArrayList<Vector>(_center_amount);
			...
			while(randomized_centers.size()<_center_amount){
				...
				for (int j = 0; j < _dimension; j++){
					data[j] = 150*rand.nextGaussian()+200;	
				}
			...
			for (int j = 0; j < randomized_centers.size(); j++) {
				for(int i = 0; i < 500; i++){
					...
					//Random[] r = new Random();
						Random r = new Random();
						if (i%2==0) data[k] = randomized_centers.get(j)._vector[k]+r.nextGaussian()*50;
						else data[k] = randomized_centers.get(j)._vector[k]-r.nextGaussian()*50;
					}
					this.vList.add(new Vector(data));
				}
			}
